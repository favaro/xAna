...
int xAna::HggTreeWriteLoop(const char* filename, int ijob  ) {
  ...
  //CF
  cout << " ijob " << ijob << " nevts " << _config->nEvtsPerJob() << endl;
  cout << " events start " << entry_start << " stop " << entry_stop << endl;
  ...
  for (Long64_t jentry=entry_start; jentry< entry_stop ; ++jentry) {
    ...
    hTotEvents->Fill(1,_minitree->mc_wPU); // fix on the fly
    ...
    //cout << " official category number " << selectedEvt.category << " -> " << finalCat << endl;
    ...
    //cout << " VBFvsgg before = " << _minitree->mela_VBFvsgg << endl;

    if( selectedEvt.category == "vbf" )  {
      evaluateMELA( selectedEvt );
      //cout << " VBFvsgg after = " << _minitree->mela_VBFvsgg << endl;
    }
    
    /*
    if( _minitree->mela_dXsec_HJJ>-999 || _minitree->mela_dXsec_HJJVBF>-999 || _minitree->mela_dXsec_HJJ_PS>-999 || _minitree->mela_dXsec_HJJVBF_PS>-999) 
      cout << " xsec " <<  _minitree->mela_dXsec_HJJ << " " <<  _minitree->mela_dXsec_HJJVBF << " " <<   _minitree->mela_dXsec_HJJ_PS << " " <<  _minitree->mela_dXsec_HJJVBF_PS << endl;

    if( _minitree->mela_VBFvsgg > -999 || _minitree->mela_SMvsPS_VBF > -999 || _minitree->mela_SMvsPS_gg > -999 )cout << "     mela = " << _minitree->mela_VBFvsgg << " " << _minitree->mela_SMvsPS_gg <<" " <<  _minitree->mela_SMvsPS_VBF << endl;
    
   
    if( _minitree->avocado > -999 || _minitree->banana > -999 || _minitree->zucca > -999 ) cout << " ******************    zucca = " << _minitree->zucca 
								    << " avocado = " <<  _minitree->avocado 
								    << " pera = " <<  _minitree->pera 
								    << " banana " << _minitree->banana << endl;
    */    

    //----- MC weights
    if( !isData ) wei = computeMCweights(selectedEvt,isprompt1);
